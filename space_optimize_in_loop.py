...
def remove_invalid_nodes():
    ...
    for parent in parents:
        children_list = parent_to_child[parent]
...
with open('rico_sca_filter.txt') as file:
    ...
    for line in lines:
      line = line.rstrip()
      parts = line.split('.')
      rico_sca_ids.append(parts[0])
    
    
# print(len(rico_sca_ids))    

parent_path = 'G:/Ph.D. Studies/Rico/unique_uis/combined/'
destination = 'G:/Ph.D. Studies/Projects/optimal-magnification/'

min_sca = int( sys.argv[1] )
max_sca = int( sys.argv[2] )

print(min_sca, max_sca)

for item in rico_sca_ids[ min_sca : max_sca + 1 ]:
  ...
  img_path = parent_path + str( id ) + ".jpg"
  json_path = parent_path + str( id ) + '.json'
  
  dest_img_path = destination + str( id ) + ".jpg"
  dest_json_path = destination + str( id ) + '.json'
  ...
  output_file_name = destination + "all_outputs/" + str(id) + "_output" + ".jpg"

  final_output.save(output_file_name)
  
  # print(output_file_name)
  
  shutil.copy( img_path, destination + "all_inputs/" )
  shutil.copy( json_path, destination + "all_inputs/" )
