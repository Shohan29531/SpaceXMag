...
#   print(index_to_coordinates)
  ...
  X = m.Array( m.Var, ( len( children ) + 1 ) * 2, lb=root.x1, ub=root.x2 + 200 )
  Y = m.Array( m.Var, ( len( children ) + 1 ) * 2, lb=root.y1, ub=root.y2 + 200 )
  ...
  for i in range(1, len(children) + 1):
    ...
    #     if ( second_child.y1 >= first_child.y1 ):
    #         relative_position_matrix[i][k] = 'T'
    #         relative_position_matrix[k][i] = 'B' 
    #     else:
    #         relative_position_matrix[i][k] = 'B'
...
for leaf in all_valid_leaves:
  ...
  dummy = trim_whitespace(old_image)
  if( dummy ):
     new_image = dummy
  else:
     node_valididty_map[leaf] = False
     print("culprit: ", leaf)
    #  new_image = old_image   
...
# print(all_valid_leaves)

all_valid_leaves_copy = all_valid_leaves.copy()

for leaf in all_valid_leaves:
    if node_valididty_map[leaf] == False:
        all_valid_leaves_copy.remove(leaf)

all_valid_leaves = all_valid_leaves_copy
